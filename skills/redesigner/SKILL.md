...
## Output layout — everything under one `redesign/` parent folder

A run **never scatters sibling folders** across the project root: **everything it generates lives
under a single `PROJECT/redesign/` parent folder**, for order and easy cleanup. The engine already
defaults to this layout — keep the paths below when invoking it.

```
PROJECT/redesign/
├── artifacts/        survey artifacts (screenshots, html, css, tokens, reports, logo, preview)
├── artifacts-after/  post-redesign mobile re-survey (optional, M.6)
├── app/              React + Vite + Tailwind + motion redesign (web build, step 9)
├── html/             static HTML mock export (step 10)
├── mobile/           React Native + Expo redesign (mobile build, M.5)
└── flows/            Maestro flows authored for the mobile survey (M.2)
```

## Capture source: web or mobile

There are **two capture sources** that both write the same `redesign/artifacts/` and feed the same downstream steps:
...
npm --prefix "SKILL_DIR" run capture -- \
  --url "<URL>" --out "PROJECT/redesign/artifacts" --max-pages 25 --no-headless
```
...
When it finishes, read `redesign/artifacts/manifest.json` (pages, `auth`, `warnings`, `skippedDestructive`). If `auth` is `failed: ...`, state the reason (e.g. manual-login timeout) and offer to retry.

## 3. Cheap scrape review (preview.html)

**Before spending context on images**, open the preview to eyeball what was captured:
```bash
open "PROJECT/redesign/artifacts/preview.html"
```
...
**Before analyzing and before asking for decisions**, generate a **navigable mock** of the site exactly as Playwright captured it, so the user can walk through what was surveyed view by view. **Delegate it to a subagent** (Agent tool) with a clean context. Pass it only the `redesign/artifacts` path. The subagent must:

- Read `redesign/artifacts/manifest.json` and the `screenshots/*.full.png`.
- Write `redesign/artifacts/site-mock/index.html`: a **self-contained, navigable** HTML file (no server, just JS + relative paths to `../screenshots/*.full.png`). Simple structure: a side panel (or topbar) with the list of pages (by `title`/`slug` from the manifest) and a main area showing the **full screenshot** of the selected view; clicking a page in the list changes the view (a "view by view" mock). Lightweight and clear, no frameworks.
- Return a short summary (which pages it included), NOT the HTML dump.

Then open it for the user:
```bash
open "PROJECT/redesign/artifacts/site-mock/index.html"
```
Tell them they can walk through the surveyed site view by view there.

## 5. Write the analysis reports

Fill in the skeletons in `redesign/artifacts/reports/` (they already exist with TODOs):
...
Look at `redesign/artifacts/logo/logo.png` and `logo/candidates/` with Read. Complete `reports/logo-analysis.md`: type (wordmark/icon/combined), quality, **is it basic/generic?** (yes/no + why).
...
2. Compose a **refined prompt for gpt-image** (concept, style, palette from `tokens.json`, transparent background, variations) and write it to `reports/logo-prompt.md`. Show it to them: **the user runs it by hand** in gpt-image/ChatGPT (no API call). They'll drop the resulting logo into `redesign/app/src/assets/`.

## 7. Expert UX audit — via SUBAGENT

**Before asking the user for decisions**, add an expert eye. **Delegate to a subagent** (Agent tool) that acts as a **senior UI/UX and product designer**. Pass it the `redesign/artifacts` paths (full screenshots + the `reports/*.md` already written). The subagent must:

- Review the screens and reports and produce an **actionable audit** in `redesign/artifacts/reports/uiux-expert-review.md`, covering: Nielsen's heuristics, visual hierarchy, consistency (buttons/spacing/color), accessibility and contrast, density and legibility, navigation patterns, states (empty/loading/error), and **quick wins** vs **redesign opportunities**.
...
current date, the `redesign/artifacts` paths (`reports/visual-style.md`, `tokens.json`,
...
3. **Recommend which trends fit THIS site/app** using `visual-style.md` + `tokens.json` +
   the UX audit — ranked.
4. Write `redesign/artifacts/reports/ui-trends.md` (human-readable) **and**
   `redesign/artifacts/trends.json` (structured): an array of trends, each
...
- **Web reference (URL):** capture it lightly — single page, no crawl:
  ```bash
  npm --prefix "SKILL_DIR" run capture -- --url "<REF_URL>" --out "PROJECT/redesign/artifacts/reference" --style-only
  ```
  Then `WebFetch` the URL for brand/vibe context. Look at
  `redesign/artifacts/reference/screenshots/*` and `reference/tokens.json`.
...
Fill `redesign/artifacts/reports/reference.md`: what it is, the aesthetic to borrow
...
```bash
npm --prefix "SKILL_DIR" run scaffold -- --out "PROJECT" --artifacts "PROJECT/redesign/artifacts"
```
This creates `PROJECT/redesign/app/` (React 19 + Vite 8 + Tailwind v4 + `motion@12`, tokens in `src/styles/theme.css`, variants in `src/lib/motion.ts`, component/page stubs).
...
- Complete the components and pages in `PROJECT/redesign/app/`, using Tailwind v4 (`@theme`) and Framer Motion (`motion/react`) generously (entrance with `fadeUp`/`staggerContainer`, hover/tap with `hoverLift`, page transitions with `AnimatePresence`).
- Verify with `npm install && npm run build` inside `redesign/app/`.
- Return a short summary (what it built, what's left), NOT all the files.

`redesign/app/` is the **source of truth** for the exports.
...
1. **Show the redesign**. Bring up the project and take screenshots of the key screens:
   ```bash
   cd "PROJECT/redesign/app" && npm run dev   # (in background) or npm run build && npm run preview
   ```
...
**Delegate ALL exports to a dedicated subagent (Agent tool) with a clean context.** This subagent does NOT need the conversation history: pass it only the paths (`PROJECT`, `redesign/artifacts`, `redesign/app/`) and the chosen export list. It must:

- **Static HTML mock**: run `npm --prefix "SKILL_DIR" run scaffold -- --out "PROJECT" --artifacts "PROJECT/redesign/artifacts" --target html` → generates `redesign/html/index.html` + `tokens.figma.json`.
...
- **Pencil (.pen)**: with the `pencil` MCP — `get_editor_state({include_schema:true})` + `get_guidelines`, then `batch_design` to recreate the redesign's key screens as a .pen design (using `redesign/app/` and the screenshots as reference); verify with `get_screenshot`.
...
Use this **instead of steps 0–6** when the source is a native app. It drives the app on a real device/emulator with Maestro (you author the flows; Maestro does the taps/waits), captures a screenshot per screen, and derives tokens/logo from the **pixels** (mobile has no DOM/CSS). Then continue with the **shared** steps 4, 5–6, 7, 8, 9–9b, 10 against the same `redesign/artifacts/`.
...
You need the app's **appId** (Android package / iOS bundleId) — ask, or read it from the project (`app.config.*`, `AndroidManifest`, etc.). Flows are small YAML files you author under a working dir (e.g. `PROJECT/redesign/flows/`). Run with `--watch` so the human can see the device and do the manual login.

**Seed the flow template first** so you edit a ready skeleton instead of hand-writing boilerplate (creates `redesign/flows/survey.yaml` + the reusable `_screenshot.yaml`; never overwrites existing flows):
```bash
...
1. **Inspect first (read-only, no flow).** Before authoring anything, dump the **current** screen's view hierarchy + a screenshot so you can read the real labels/ids and author reliable selectors in ONE pass instead of guessing:
   ```bash
   npm --prefix "SKILL_DIR" run mobile-inspect -- --platform android|ios --out "PROJECT/redesign/artifacts"
   ```
...
npm --prefix "SKILL_DIR" run mobile-capture -- \
  ...
  --flows "<flow .yaml or dir>" --out "PROJECT/redesign/artifacts" --watch
```
...
```bash
npm --prefix "SKILL_DIR" run mobile-scaffold -- --out "PROJECT" --artifacts "PROJECT/redesign/artifacts"
```
...
2. Author a small Maestro flow that walks the redesign and re-capture it: `mobile-capture --app <redesign app id> --flows <flow> --out PROJECT/redesign/artifacts-after`. Read those screenshots and show them next to the original survey screens (`redesign/artifacts/screens/`).
...
`AskUserQuestion`: which exports? For a native app the main target is **Pencil (.pen)** as phone frames. Delegate to a subagent with a clean context (pass only the paths: `PROJECT`, `redesign/artifacts`, `redesign/mobile/`).
...
- The **split** (before/after comparison) is **embedded in `redesign/app/`** via `CompareShell` wrapping the app, not a separate artifact: it uses the original HTML copied to `public/_original/` (with the `assets/` downloaded during capture) and the real redesign as "after". It needs the redesign server running (`npm run dev`).
